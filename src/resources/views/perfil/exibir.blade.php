...
<div class="container-perfil">
   ...
   <div class="capa-perfil-banner">
        <img src="{{ $usuario->perfil && $usuario->perfil->capa ? asset('storage/'.$usuario->perfil->capa) : asset('imagens/capa-padrao-perfil.png') }}">
    </div>
...
<div class="abas-perfil">
    <button class="aba ativa" data-tab="atividade">Atividade</button>
    <button class="aba" data-tab="capitulos">Capítulos</button>
    <button class="aba" data-tab="salvos">Salvos e curtidos</button>
</div>

<!-- Conteúdo 1: Atividade (Aba Ativa Inicialmente) -->
<div class="conteudo-aba" id="aba-atividade">
    @if($ehMeuPerfil)
    ...
    <div class="lista-posts-perfil">
            ...
            <article class="cartao-post-perfil">
                @if($post->categorias)
                    <div class="categorias-post-perfil">
                        @foreach(explode(',', $post->categorias) as $categoria)
                            <span class="categoria-badge-perfil">{{ trim($categoria) }}</span>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="corpo-post-perfil">
                    <div class="texto-post-perfil">
                        <h3><a href="{{ route('publicacao.detalhes', $post->id_publicacao) }}">{{ $post->titulo }}</a></h3>
                        <p>{{ $post->resumo }}</p>
                        <a href="{{ route('publicacao.detalhes', $post->id_publicacao) }}" class="link-ver-mais-perfil">Ver mais</a>
                    </div>
                    @if($post->capa)
                ...
                <div class="acoes-post-perfil">
                    @php $ja_curtiu = $post->curtidas->where('fk_id_usuario', Auth::id())->first(); @endphp
                    <button type="button" class="botao-acao-perfil btn-curtir {{ $ja_curtiu ? 'curtido' : '' }}" data-id="{{ $post->id_publicacao }}" data-token="{{ csrf_token() }}" onclick="alternarCurtida(this)">
        ...
        @empty
            <p class="sem-postagens">Nenhuma publicação encontrada.</p>
        @endforelse
    </div>
</div>
...
<div class="conteudo-aba" id="aba-salvos" style="display: none;">
    <div class="container-botoes-filtro">
        <div class="grupo-botoes-alternancia">
            ...
            <button type="button" class="btn-filtro-aba" data-subtab="lista-curtidos">
    ...
    <!-- Sub-painel: Salvos -->
    <div class="painel-conteudo-aba" id="lista-salvos">
        <div class="lista-posts-perfil">
            @forelse($usuario->salvos ?? [] as $itemSalvo)
    ...
    <!-- Sub-painel: Curtidos -->
    <div class="painel-conteudo-aba" id="lista-curtidos" style="display: none;">
        <div class="lista-posts-perfil">
            @forelse($usuario->curtidas ?? [] as $itemCurtida)
...
<div class="modal-overlay" id="modal-editar-perfil">
    <div class="modal-container modal-editar-container">
        ...
        <div class="modal-cabecalho">
            ...
            <h2>Editar Perfil</h2>
            <div class="espacador-cabecalho"></div>
        </div>

        <form action="{{ route('perfil.atualizar', $usuario->id_usuario) }}" method="POST" enctype="multipart/form-data">            
            @csrf
            
            <div class="imagens-modal-wrapper">
                
                <div class="modal-capa-preview-container">
                    <img src="{{ $usuario->perfil && $usuario->perfil->capa ? asset('storage/' . $usuario->perfil->capa) : asset('imagens/capa-padrao-perfil.png') }}" id="capa-preview">
                    
                    <label for="capa_input" class="botao-upload-lapis botao-lapis-capa" title="Mudar Capa">
                        <i class="fa-solid fa-pencil icone-lapis-capa"></i>
                    </label>
                    <input type="file" id="capa_input" class="input-file-oculto" name="capa" accept="image/*" onchange="previewCapa(this)">
                </div>

                <div class="container-avatar-upload">
                    <div class="avatar-preview-wrapper">
                        <img src="{{ $usuario->perfil && $usuario->perfil->foto ? asset('storage/' . $usuario->perfil->foto) : asset('imagens/perfil-v1.png') }}" id="avatar-preview" alt="Avatar">
                        
                        <label for="foto_input" class="botao-upload-lapis botao-lapis-avatar" title="Mudar Foto">
                            <i class="fa-solid fa-pencil icone-lapis-avatar"></i>
                        </label>
                        <input type="file" id="foto_input" class="input-file-oculto" name="foto" accept="image/*" onchange="previewImagem(this)">
                    </div>
                </div>
